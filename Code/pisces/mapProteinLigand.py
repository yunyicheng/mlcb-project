...
def mapProteinLigand(i):
    ...
    adata.obs["section"] = (
        adata.obs["x_bin"].astype(str)
        + "_"
        + adata.obs["y_bin"].astype(str)
        + "_"
    ...
    cell_metadata_file = (
        DATADIR / "metadata/MERFISH-C57BL6J-638850/20241115/cell_metadata.csv"
    )
    ...
    for brain_section, indices in brain_section_groups.groups.items():
        ...
        print(
            f"Created AnnData for brain section '{brain_section}' with {brain_section_adata[brain_section].n_obs} cells."
        )

    # Dictionary to store graphs for each brain section
    brain_section_graphs = {}
    brain_section_code = list(brain_section_adata.keys())
    brain_section = brain_section_code[i]
    ...
    for spatial_section, indices in spatial_section_groups.groups.items():
        ...
        print(
            f"Created AnnData for spatial section '{spatial_section}' with {spatial_section_adata[spatial_section].n_obs} cells."
        )
    ...
    for section_label in spatial_section_adata:
        ...
        filtered_protein_ligand_data = protein_ligand_data[
            ...
            & (
                protein_ligand_data["receptor_ensembl_gene_id"].isin(
                    adata_genes_spatial
                )
            )
        ]
        ...
        for cell_idx, cell_id in enumerate(cell_ids):
            ...
            avg_expression = np.mean(
                expression_matrix[cell_idx, :].toarray()
            )  # Use `.toarray()` if sparse
            G.add_node(cell_id, average_expression=avg_expression)

        # Map gene names to column indices in the expression matrix
        gene_to_idx = {
            gene: idx for idx, gene in enumerate(adata_spatial.var["gene_symbol"])
        }
        gene_id_to_symbol = dict(
            zip(adata_spatial.var.index, adata_spatial.var["gene_symbol"])
        )
        ...
        for _, row in tqdm(filtered_protein_ligand_data.iterrows()):
            ...
            if ligand_idx is not None and receptor_idx is not None:
                ...
                for i, j in tqdm(cell_pairs, desc="Cell Pairs"):
                    ...
                    if (
                        ligand_expr > 0 and receptor_expr > 0
                    ):  # Check for non-zero expression
        ...
        output_path = (
            Path(GRAPHOUTDIR)
            / f"{brain_section}_{section_label}_cell_ligand_receptor_graph.gpickle"
        )
